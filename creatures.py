...
class Creature(object):
	...
	def __str__(self):
		desc = ""
		if name:
			desc+="You see %s"%(self.name)
		desc+="It's a %s %s"%(self.combat_class, self.race)
		desc+="It has %d health and %d energy"%(self.health, self.energy)
...
class Player(Creature):
	...
	def __str__(self):
		desc = super().__str__()
		desc += "It is of level %d"%(self.level)